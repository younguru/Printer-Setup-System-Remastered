...
<%
	AuthorizeUtil.UserLoadedJspRedirect(request, response, "AdminInstallScriptCreate.jsp", "/admininstallscriptcreate");
	...
	<body class="container-fluid" style="min-width: ${MinWidth};">
		...
		<div class="row ml-4 mr-4 mt-3 mb-4">
			<div class="col-3 text-truncate p-4 border-right">
				...
				<p class="mb-1"><img class="size-16" src="img/admin/installscript.png" alt="Install Script page settings"/><a class="ml-2 text-body font-weight-bold" href="${context}/admininstallscript"><u>${Installscript}</u></a> -><img class="size-16" src="img/admin/create.png" alt="Create Script"/><a class="ml-2 text-body font-weight-bold" href="${context}/admininstallscriptcreate"><u>${Createscript}</u></a></p>
...
<%--				</ul>--%>
				...
				<div class="mb-3">
					<form method="post" action="admininstallscriptcreate">
						<label for="inputNameInput">${Scriptname}</label>
						<input class="form-control mb-4" id="inputnameInput" type="text" name="adminscriptname" value="${script.getName()}">
						<label for="inputExtensionInput">${Scriptextension}</label>
						<input class="form-control mb-4" id="inputExtensionInput" type="text" name="adminscriptextension" value="${script.getExtension()}">
						<label for="adminscriptInput">${Script}</label>
						<textarea class="form-control" name="adminscript" id="adminscriptInput" class="w-100" rows="15" autocomplete="off" autocapitalize="off" spellcheck="false">${script.getValue()}</textarea>
						<div class="form-group hidden">
							<input name="editscriptid" type="text" class="form-control hidden" value="${script.getId()}">
						</div>
						<button name="button_savescript" type="submit" class="btn btn-outline-primary mt-3">${Save}</button>
					</form>
				</div>
			</div>
		</div>
